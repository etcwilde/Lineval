Internally, git uses a directed acyclic graph model (DAG) with references
pointing from the most recent commit \notice{(TODO: most
    recent commit?, it's not the \emph{HEAD} because that moves
    around)} toward the initial commit in the repository. This model is
...
% \evan{Change this when we get the actual results, this is from the
% pilot study} We find that although the DAG view used by Gitk provides
% more information about the repository, in many cases it povides too
% much information and overwhelms the user.

% \evan{Move this} In some cases, where the structure of the merge tree
% is flat, that is to say that it has no inner merges, users are able to
% easily determine which commits are merged together, but are unable to
% determine which merge is the final merge is the root of the tree.
