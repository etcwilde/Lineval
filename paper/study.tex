...
Gitk. T1 has the participant draw a diagram of the merge tree,
...
which merges the participant has defined to be in the merge tree; second
...
We include two commits in the study to detect differences between \mt
sizes. The order that the commits are presented to each participant is
randomized. We analyzed the 15096 merge-trees from April 16th 2005 to
...
non-root nodes. 75\% of the trees contain at least 51 nodes, and the
largest tree contains 7217 nodes.
...
the size of the merge-tree, but the accuracy is not. The correctness
