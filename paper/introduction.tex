...
opensource and closedsource projects. Git enables many developers to
...
time. Git refers to nodes of the DAG as comits, not distinguishing
...
refereing to all as repository events.
...
\begin{figure}
  ...
  \caption{
    ...
    showing the commit dates. The bottom-leftpane shows the full
...
have a DAG visualzation to show the branches and forks of a
repositority, but is unable to display the visualization for projects at
...
This paper investigates the construction, implementation, user-based
evaluation, and generalization of a new model, the \mt model. We
begin with a description and algorithm for converting the DAG into a set
of merge trees. We continue by providing some details about the
web-based tool, called \tool\footnote{\tool is currently available at
  \url{http://li.turingmachine.org}}, implemented to test the
feasibility and potential capabilities of the \mt model. We
continue with a user study, to evaluate first, if the DAG is capable of
providing the users with a conceptual understanding of the repository
events that are related to a given commit, and second, determine if
there is a difference between the DAG and \mt model in performing
summarization tasks.

\subsection{Directed Acyclic Graph Model}
\label{sec:directed_acyclic_graph_model}

Git uses the DAG as the internal model for representing repositories.
This model functions perfectly, and is necessary, for the internal
functionality of Git. The DAG model allows for the reconstruction of
files and the working directory by recursively applying the changes, or
the diff, made in each repository event that is a parent of the current
event. We see an example DAG repository Figure~\ref{fig:repoDAG}.

Each repository event contains an ordered list of one or more parents,
except the initial commit, which has no parents. Commits have a single
parent, while merges will have at least two parents, the current branch,
and the branch being merged into the current branch. Git allows for the
merging of multiple branches simultaneously in octopus merges, where the
order of the parents represents the order that the branches were
specified for being merged.

The repository events in the DAG are immutable; once an event is
created, it can't be changed. Git allows operations to alter commits and
re-order them, but this will create a new commit with a new commit hash.
This property makes a repository event unable to record the traversal of
merges into the master branch of the repository. A commit cannot have
references in the direction of the merge that merges the commit, as this
reference will change, requiring the commit to be changed. While this
provides some challenge, git eliminates this issue of the DAG with the
\verb|git log --children| command, which traverses the DAG and inverts
the edges to provide the reference from parent to child. This
terminology will prove to be confusing through the rest of the paper,
when we are referring to the \mt form, the parent is the next node
toward the root. When referring to the DAG, the parent relationship is
in the opposite direction, from the root toward the branch-point.

Many current visualization systems base the visualization on the DAG,
because it is made readily available by git. The primary issue being
that the DAG was not designed with visualization and summarization tasks
in mind. Due to the direction of the links, the DAG model is unable to
provide a summarization of the contents of a merge; the DAG is unable to
show which branch is the master branch, and is unable to filter the
irrelevant information from the view.

\subsection{\mt Model}
\label{sub:merge_tree_model}

We use the \mt model to address these issues in the DAG\@. We
will describe the \mt model in detail in
Section~\ref{sec:mergetree}. The \mt reverses the direction of
the references of the DAG.\@ Starting with each merge into the master
branch, the algorithm recursively aggregates the statistics of each
commit. The tree is rooted at the merge into the master branch, and the
commits are the leaves.
...
The \mt model inverts the DAG, so that the reference point from
parent to child instead of child to parent. This is to enable
aggregation at merges. Given a merge, we are able to recursively
traverse all children of that merge, collecting the statistics at each
commit. The inverted DAG is then traversed starting at each merge into
the master branch. The merges into the master branch is the root of each
tree. The commits are the leaves of the tree. Starting at the merge into
the master branch, we recursively traverse each child of the current
node until we reach the branch point from the master branch, or a commit
that has a possible root merge that is closer in commit time to the
commit. As a consequence, commits will always take the shortest path, in
terms of time, to the master branch.
...
% TODO: make everything one color
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[auto, on grid, semithick, state/.style={circle, text=black}]
    \node[state, draw=chartblue] (1) {1};
...
\subsection{Computing the \mt from the DAG of Linux}
...
Merges that do not have merge conflicts provide information to verify
this heuristic. If a merge does not contain a conflict, it records a
summary of the commits that it merges. See Figure~\ref{fig:sampleMerge}
for an example. This summary contains a list of the first 20 non-merge
commits in the merge, including their one-line log description, the full
logs of the merge commits that merge this subset, and the total number
of non-merge commits in the merge.
...
We used this information to evaluate the accuracy of the \mt
...
root of a \mt. As described above, the log of a merge-commit
...
the  \mt rooted in this commit. We also used the summaries of the
...
one-line summaries corresponds to commits in that \mt. We also
