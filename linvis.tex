...
\begin{abstract}
        ...
        In this paper, we propose the merge-tree view, a simplified transformation
        of the DAG of the Linux git repository that shows the way in which commits
        are merged into the master branch of Linux. Using the merge-tree view we
...
at the scale of the Linux of the kernel (Figure~\ref{fig:gitfail}).  Between 60 and
...
\begin{figure}
        ...
        \caption{Search View. It allows finding commits that were \textbf{merged} in
                a given period or by commit it.}
        \label{fig:search}
\end{figure}
...
version. A user can further narrow down the search by \textbf{specifying a range of
dates} in which such commits \textbf{were merged by Linus into master, not when the
commits were created.} This distinction is important. We have observed commits that
have taken years to arrive into master after they were originally committed.
...
If providing commit ID must be specified from the first character to the last
character. For example, the commit `c267548755a184ef97301071300c1739a564e135' is
returned when the user searches for a commit ID of `c26' in the 3.10 Linux kernel.
This commit is a non-merge commit performed, in one of the largest merge-trees of
the kernel; however, this feature will work with any commit ID.  We will use this
commit for showing each of the features of \tool.

\begin{figure}
        \centering
        \includegraphics[width=0.47\textwidth]{figures/search_results_2.png}
        \caption{Search Results. Each is a commit merged in the desired merged
                window.}
        \label{fig:results}
\end{figure}
...
\begin{figure}
        ...
        \caption{Panels showing the files modified by all the commits that are part
                of this merge.}
        \label{fig:files}
\end{figure}
...
heuristic approach fails. As with the Files panels, non-merge commits show their
...
The tree can be further extended to include the collapsing of merge nodes. Clicking
on merge nodes will cause them to collapse, only showing parts of the tree that are
of interest to the user.Furthermore, the user can zoom the tree by scrolling the
mouse wheel to see more of the topology of the tree, or to see the details of a
given part of the tree.
...
The biggest challenge, however, is computing the merge-tree. Our heuristic
\textbf{only} applies to the Linux kernel repository, and only since July 2009. It
is very likely that the heuristic will not work with other repository, specially
those with fast-forward commits into the master branch (a majority of repositories).
We can address the lack of information by monitoring the state of the repository at
short intervals as we did in \cite{German2015}. This continuous mining of the
repository requires foresight and planning.
...
kernel that the user is interested in. In Figure~\ref{fig:dag_view} and
