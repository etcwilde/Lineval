...
\evantodo {
        \begin{enumerate}
                \item Add simplified abstract -- From email
                \item replace figure reference with Figure~\\ref
                \item Check captions
                \item gitk to Gitk
                \item git to Git
                \item diff to patch
...
presence of \textit{foxtrot} commit, which confounds the true master branch of a
repository \footnote{See
        \url{http://bit-booster.blogspot.ca/2016/02/no-foxtrots-allowed.html} for a
        description of the issue}, and will not work for older merges.
